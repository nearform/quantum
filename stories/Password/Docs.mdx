...
`size` sets the height of the field — `'sm'` (37px) or `'default'` (42px) — and is the same prop, with the same two heights, as [Input](?path=/docs/form-input--docs)'s, so a password and a text field line up in a form. It displaces the HTML attribute of that name, which the field's layout was overruling anyway.
