...
The attributes for the Search Input are identical to that of the `<HTMLInputElement>` type. A caveat is that, if one were to pass in the `value` attribute, the clear button would not be able to clear the text area.
