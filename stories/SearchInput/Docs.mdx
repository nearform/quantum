  ...
  return (
    <div className="flex flex-col space-y-1">
      <SearchInput
        ...
        className="peer" // <-----Note that we use `peer` here.
        ref={focused}
      />
      {//Below you can see we use the peer-focus-within attribute. That's because the SearchInput is not a pure input, but a div that contains the input element. }
